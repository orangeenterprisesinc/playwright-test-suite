...
| Artifact | Path |
|---|---|
| This plan | `specs/system/login.md` |
| Spec | `tests/ui/system/login-module.spec.ts` |
| Runner rows | `src/data/runner/system.csv` → `UI-001`…`UI-004` |
| Value bag | `src/data/system/loginModuleData.ts` |
...
**literal** id in the `testCaseId` annotation (`scripts/lib/runner-data.js` →
...
- **Invalid values and the expected error** —
  [`loginModuleData.ts`](../../src/data/system/loginModuleData.ts).
