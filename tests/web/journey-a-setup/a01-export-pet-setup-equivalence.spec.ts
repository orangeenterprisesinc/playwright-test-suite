/**
 * Equivalence: export-pet-setup — Catalog workflow **A1 — User & core setup**
 * equivalence tail (WEBPET-845, Export Engine Slice 12).
 *
 * | | |
 * |---|---|
 * | Plan | `test-plans/journey-a/a01-user-setup.md` |
 * | Runner rows | `src/data/runner/journey-a.csv` → `A1-008` |
 *
 * Relocated from `tests/webpet/equiv/export-pet-setup-equivalence.spec.ts`
 * (WP-0176). Every assertion below is the one that spec carried, in the same
 * order; what changed is the fixture (`base.fixture`), the auth transport, and
 * the id/tag vocabulary.
 *
 * ## Why `sessionApi` replaces `csrfFromStorage()` + `WEBPET_ADMIN_STORAGE`
 *
 * The source authenticated by reading the `pt_csrf` cookie out of
 * `WEBPET_ADMIN_STORAGE` and attaching it by hand to `page.request.post(...)`.
 * That storage file is refreshed only by the `webpet-setup` project, which
 * journey projects never run — keeping it would recreate the exact
 * stale-storage trap that took `A7-051` down in batch 8: a guard whose
 * file-existence check stopped implying a live session. `sessionApi`
 * (`base.fixture`) already carries the session cookie and `X-CSRF-Token` on
 * every call, so the helper and both config imports it needed are gone.
 * Precedent: `tests/web/journey-b-field/b03-scan-time-in-equivalence.spec.ts`.
 *
 ...
 * docs).
 *
 * This test is quarantined (`enabled=0`, host-bound): it diffs against a
 * baseline produced by the legacy WinForms app on the windows-automation host
 * at `C:\Scripts\*.yaml`, which no CI runner has.
 */
...
test.describe('Equivalence: export-pet-setup (web export vs legacy PET-Setup)', { tag: ['@JourneyA', '@A1'] }, () => {
    test.skip(!EQUIV_ENABLED, SKIP_REASON);

    // Quarantined (row enabled=0, host-bound): diffs against a baseline
    // produced by the legacy WinForms app on the windows-automation host at
    // C:\Scripts\*.yaml, which no CI runner has.
    test('[Equiv] Verify that the web export matches legacy PET-Setup and round-trips through import.', {
        tag: ['@Regression'],
        annotation: [
            { type: 'testCaseId', description: 'A1-008' },
            { type: 'requirement', description: 'A1-R11|A1-R12' },
        ],
    }, async ({ page, sessionApi }) => {
        test.setTimeout(300_000);

        // ── Trigger the web export via the new engine ────────────────────────────
        const exportRes = await sessionApi.post('/api/connectivity/export/scan-devices', {
            data: {},
        });
        ...
        const importRes = await sessionApi.post('/api/connectivity/import/single-folder', form);
        ...
        let terminal = '';
        for (let i = 0; i < 30; i++) {
            const runRes = await sessionApi.get(`/api/connectivity/import/runs/${importBody.runId}`);
            if (runRes.ok()) {
