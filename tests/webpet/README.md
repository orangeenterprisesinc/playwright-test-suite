...
|---|---|---|
| `E2E_ADMIN_USER` / `E2E_ADMIN_PASSWORD` | admin (`su`) API login in [support/provision.ts](support/provision.ts) + notifications.spec.ts; falls back to `USER_NAME`/`PASSWORD` (see [src/config/webpetEnv.ts](../../src/config/webpetEnv.ts)) so a local `test:webpet:dev` reuses the framework's dev credentials | CI secret `DEV_PASSWORD` / fallback: `.env.dev` + `.env` |
...
| `S3_ENDPOINT` | non-empty ⇒ employee-documents.spec.ts runs (needs object storage) | deliberately absent on dev, so the spec skips |
