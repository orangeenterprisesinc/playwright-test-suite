 ...
 *
 * Keep the interpreter resolution here in sync with src/video/videoProcessor.ts
 * — this plain-JS copy exists for use before/outside the TS build (npm scripts
 * and CI, where ts-node isn't guaranteed to be available), the same reason
 * scripts/generate-allure-report.js mirrors src/utils/allureHelper.ts. All
 * detection logic lives in tools/video-annotator/annotate_video.py; both entry
 * points just invoke it.
 *
 * Unrecognised flags are forwarded to the annotator, so `--scene-threshold`,
 * `--settle-ms`, `--max-frames` and friends work here without being restated.
 *
 * Usage:
...
function printSummary(outputDir) {
    ...
    console.log([
        ...
        '      Reads this output and drafts a test plan under specs/, then generates the spec.',
        '',
    ].join('\n'));
}
