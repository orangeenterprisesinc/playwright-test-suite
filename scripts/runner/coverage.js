 ...
 *   - the last run (`artifacts/results/results.json`) — what actually passed
 *
 * Per-workflow state:
...
const RESULTS = path.join(ROOT, 'artifacts', 'results', 'results.json');
const SPECS_DIR = path.join(ROOT, 'test-plans');
...
function main() {
    ...
    if (!outcomes.size) {
        console.log("No artifacts/results/results.json — run the suite for 'passing' state.");
    }
}
