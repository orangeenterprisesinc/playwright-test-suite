...
function main() {
    ...
    for (const row of rows) {
        ...
        // The spec's folder under tests/ must be the one its category maps to.
        for (const spec of claimedBy) {
            const folder = spec.split('/')[1];
            const expected = CATEGORY_FOLDER[row.category];
            if (folder !== expected) {
                fail(`[${row.id}]: category '${row.category}' belongs in tests/${expected}/ but the spec lives in tests/${folder}/ (${spec})`);
            }
        }
