...
class EmailReporter implements Reporter {
    ...
    async onEnd(result: FullResult): Promise<void> {
        ...
        if (!host || !from) {
            this.logger.warn('SEND_EMAIL=yes but SMTP_HOST / EMAIL_FROM are not both set — skipping email');
            return;
        }

        // Built before the recipient lookup, not after: routing keys off this run's
        // branch and trigger, and those only exist on the summary.
        const summary = this.collector.build(result);
        const to = await resolveRecipients(summary.branch, summary.trigger);
        if (!to) {
            // resolveRecipients has already logged which case this is — a
            // deliberately empty row, or a table it could not resolve at all.
            return;
        }
