---
name: annotate-video
description: Use when the user asks to annotate or process a screen recording of a manual test journey (e.g. "annotate the video in Testing video", "run the annotator on this recording") — runs the OpenCV frame-diff annotator over the video and writes timestamped annotations plus rendered keyframes. Stage 1 of two; stops before any test planning or code generation.
---
...
The argument may be a video file **or** a directory (`Testing video/` is the
usual one).

- Directory → glob for `*.mp4` and `*.webm`.
