...
.video-annotations/<slug>/
   ...
   ▼
specs/<slug>.md -> playwright-test-generator -> tests/ui/<module>.spec.ts
```
...
.venv/Scripts/python annotate_video.py \
    --input "../../Testing video/Journey A1 User Setup (1).mp4" \
    --output-dir ../../.video-annotations/journey-a1
```
...
| `.github/workflows/annotate-video.yml` | Runs that image against a video URL, publishes the output as an artifact |
...
```bash
ffmpeg -i test-results/<test>/video.webm -c:v libx264 -pix_fmt yuv420p out.mp4
```
