...
2. **The dotenv loader** [`src/config/envLoader.ts`](../src/config/envLoader.ts) runs at the
   ...
   ```
3. **Per-env URLs** live in the env files: `.env.local` → `http://localhost:8090`
   (+`/api`, same origin — see [docker/e2e/](../docker/e2e/README.md)); `.env.dev` /
   `.env.qa` → the dev/qa hosts (+`/api`). The env
...
| **Containerized stack** | `TEST_ENV=local` → `.env.local` points at [docker/e2e/](../docker/e2e/README.md) (`localhost:8090`, Caddy serves the SPA and proxies `/api`). Compose sets the same values as OS env for the in-network `tests` service, which wins over the file. |
...
The self-hosted `e2e-local.yml` / `webpet-e2e-local.yml` pair was **removed** — they booted the
app natively on the QA machine (SQL Server Express over Windows Integrated Auth, `go build`,
`pnpm dev --port 3000`), which only ever worked on one host. Local runs now go through the
containerized stack in [docker/e2e/](../docker/e2e/README.md); it has no workflow yet.
